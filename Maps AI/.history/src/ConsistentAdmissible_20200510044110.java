...
public class ConsistentAdmissible {
        ...
        while (!queue.isEmpty()) {
            ...
            for (Location current : vertex.getNeighbors().keySet()) {
                ...
                if (!visited.contains(current)) {
                    ...
                    visited.add(current);
                    queue.add(current);
                    isConsistent(current, goal, pathSum + current.getG_n()); // sumando el path
    ...
    public boolean isConsistentHelper(Location start, Location goal, int pathSum) {
        return isConsistent(start, goal, start.getG_n()); //pa que se sume solo una vez
    }
